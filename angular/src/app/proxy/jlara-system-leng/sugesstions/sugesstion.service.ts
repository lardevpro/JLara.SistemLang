...
export class SugesstionService {
  ...
  getList = (input: SugesstionGetListInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<SugesstionDto>>({
      method: 'GET',
      url: '/api/app/sugesstion',
      params: { userId: input.userId, sugesstionText: input.sugesstionText, sugesstionCreationDate: input.sugesstionCreationDate, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
