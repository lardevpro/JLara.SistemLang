...
export class ExerciseService {
  ...
  getList = (input: ExerciseGetListInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ExerciseDto>>({
      method: 'GET',
      url: '/api/app/exercise',
      params: { userId: input.userId, phrase: input.phrase, difficultyLevel: input.difficultyLevel, focusArea: input.focusArea, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
